public final class Player extends Playable {
	...
	protected boolean checkDoCastConditions(L2Skill skill) {
		...
		if (skill.getSkillType() == L2SkillType.SUMMON) { // Can't summon multiple servitors.
			if (!((L2SkillSummon) skill).isCubic() && (getPet() != null || isMounted())) {
				sendPacket(SystemMessageId.SUMMON_ONLY_ONE);
				return false;
			}
		} else if (skill.getSkillType() == L2SkillType.RESURRECT) { // Can't use ressurect skills on siege if you are defender and control towers is not alive, if you are attacker and flag isn't spawned or if you aren't part of that siege.
		...
		} else if (skill.getSkillType() == L2SkillType.SIGNET || skill.getSkillType() == L2SkillType.SIGNET_CASTTIME) { // Can't casting signets on peace zone.
		...
		if (isInOlympiadMode() && (skill.isHeroSkill() || skill.getSkillType() == L2SkillType.RESURRECT)) { // Can't use Hero and resurrect skills during Olympiad
	...
	public boolean doDie(Creature killer) {
		if (!super.doDie(killer)) // Kill the Player
			return false;
	...
	public void handleCursedWeaponOnDie(Player pk, Creature killer) {
	...
	public void killerPlayerOnDie(Creature killer, Player pk, int playerObjectId, boolean playerMinLevel) {
	...
	private void onDieDropItem(Player pk, Creature killer, boolean isPlayer) {
		if (isPlayer) {
...
//				spreeCounterMsg();
	...
	public int calculateGoldForAllAvailableSkills(List<GeneralSkillNode> skills) {
		...
		for (GeneralSkillNode availableSkill : skills) {
			...
			for (int level = getSkillLevel(skillId) + 1; level <= availableSkill.getValue(); level++) {
	...
	public int calculateGoldSpentOnLearnedSkills(List<GeneralSkillNode> skills) {
		...
		for (GeneralSkillNode availableSkill : skills) {
			...
			for (int level = 1; level <= getSkillLevel(skillId); level++) {
				GeneralSkillNode skillNode = getTemplate().findSkill(skillId, level);
	...
	public boolean disarmWeapons() {
		...
		ItemInstance wpn = getInventory().getPaperdollItem(Inventory.PAPERDOLL_RHAND); // Unequip the weapon
		...
		ItemInstance sld = getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND); // Unequip the shield
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove) {
		// ************************************* Check Player State *******************************************
		if (isDead() || isOutOfControl()) { // Check if the player is dead or out of control.
			sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		...
		if (isFishing() && (sklType != L2SkillType.PUMPING && sklType != L2SkillType.REELING && sklType != L2SkillType.FISHING)) { // Only fishing skills are available
		...
		if (isSitting()) { // Check if the caster is sitted.
			sendPacket(SystemMessageId.CANT_MOVE_SITTING);
			sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		
		if (skill.isToggle()) { // Check if the skill type is TOGGLE	
			L2Effect effect = getFirstEffect(skill.getId());
			
			if (effect != null) {
				if (skill.getId() != 60) // If the toggle is different of FakeDeath, you can de-activate it clicking on it.
					effect.exit();
		...
		if (isFakeDeath()) { // Check if the player uses "Fake Death" skill
			sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
		
		// ************************************* Check Target *******************************************
		WorldObject target = null; // Create and set a WorldObject containing the target of the skill
		...
		if (target instanceof Door) {
			if (!((Door) target).isAutoAttackable(this) || (((Door) target).isUnlockable() && skill.getSkillType() != L2SkillType.UNLOCK)) { // Siege doors only hittable during siege or Unlockable doors
				sendErrorPackets();
				return false;
			}
		}
		
		if (target.isInstanceOfPlayer() && !((Player) target).isDead()) { // Fake death
			if (((Player) target).isAlikeDead() || ((Player) target).isFakeDeath()) {
				sendMessage("You can't attack already dead player!");
				sendErrorPackets();
				return false;
			}
		}
		
		if (isIncorrectTarget(target)) { // Cannot attack your own outpost.
			sendErrorPackets();
			return false;
		}
		
		if (isInDuel()) { // Are the target and the player in the same duel?
		...
		if (skill.isSiegeSummonSkill()) { // Siege summon checks. Both checks send a message to the player if it return false.
			final Siege siege = CastleManager.getInstance().getActiveSiege(this);
		...
		if (!skill.checkCondition(this, target, false)) { // Check if all casting conditions are completed
			sendPacket(ActionFailed.STATIC_PACKET); // Send ActionFailed to the Player
			return false;
		}
		
		// ************************************* Check Skill Type *******************************************
		if (skill.isOffensive()) { // Check if this is offensive magic skill
			if (isInsidePeaceZone(this, target)) {
				sendPacket(SystemMessageId.TARGET_IN_PEACEZONE); // If Creature or target is in a peace zone, send a system message TARGET_IN_PEACEZONE ActionFailed
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
			}
			
			if (isInOlympiadMode() && !isOlympiadStart()) {
				sendPacket(ActionFailed.STATIC_PACKET); // if Player is in Olympia and the match isn't already start, send ActionFailed
				return false;
			}
			
			if (!target.isAttackable() && !getAccessLevel().allowPeaceAttack()) {
				sendPacket(ActionFailed.STATIC_PACKET); // If target is not attackable, send ActionFailed
				return false;
			}
			
			if (!target.isAutoAttackable(this) && !forceUse) { // Check if a Forced ATTACK is in progress on non-attackable target
			...
			if (dontMove) { // Check if the target is in the skill cast range
				if (sklTargetType == SkillTargetType.TARGET_GROUND) { // Calculate the distance between the Player and the target
					if (!isInsideRadius(worldPosition.getX(), worldPosition.getY(), worldPosition.getZ(), (int) (skill.getCastRange() + getCollisionRadius()), false, false)) {
						sendPacket(SystemMessageId.TARGET_TOO_FAR); // Send a System Message to the caster
						sendPacket(ActionFailed.STATIC_PACKET); // Send ActionFailed to the Player
						return false;
					}
		...
		if (!skill.isOffensive() && !forceUse && (target instanceof Monster || target instanceof SiegeNpcBase)) { // Check if the skill is defensive
			switch (sklTargetType) { // check if the target is a monster and if force attack is set.. if not then we don't want to cast.
				case TARGET_PET:
				case TARGET_SUMMON:
				case TARGET_AURA:
				case TARGET_FRONT_AURA:
		...
		if (sklType == L2SkillType.SPOIL) { // Check if the skill is Spoil type and if the target isn't already spoiled
		...
		if (sklType == L2SkillType.DRAIN_SOUL) { // Check if the skill is Drain Soul (Soul Crystals) and if the target is a MOB
			if (!(target instanceof Monster)) {
				sendErrorPackets();
				return false;
			}
		...
		switch (sklTargetType) { // Check if this is a Pvp skill and target isn't a non-flagged/non-karma player
			case TARGET_PARTY:
			...
			default:
				if (!checkPvpSkill(target, skill) && !getAccessLevel().allowPeaceAttack()) {
					sendPacket(SystemMessageId.TARGET_IS_INCORRECT); // Send a System Message to the Player
					sendPacket(ActionFailed.STATIC_PACKET); // Send ActionFailed to the Player
					return false;
				}
		}
		
		if ((sklTargetType == SkillTargetType.TARGET_HOLY && !canCastSealOfRule(CastleManager.getInstance().getCastle(this), false, skill, target)) || (sklType == L2SkillType.SIEGEFLAG && !L2SkillSiegeFlag.checkIfOkToPlaceFlag(this, false)) || (sklType == L2SkillType.STRSIEGEASSAULT && !checkIfOkToUseStriderSiegeAssault(skill)) || (sklType == L2SkillType.SUMMON_FRIEND && !(canSummon() && isTargetSummonable(target)))) {
			sendPacket(ActionFailed.STATIC_PACKET);
			abortCast();
			return false;
		}
		
		if (skill.getCastRange() > 0) { // GeoData Los Check here
	...
	private boolean isIncorrectTarget(WorldObject target) {
		switch (getFactionId()) {
			case Constant.GOD: return target instanceof GuardGod;
			case Constant.GIANT: return target instanceof GuardGiant;
			case Constant.TITAN: return target instanceof GuardTitan;
	...
	private void updatePunishState() {
		if (getPunishLevel() != PunishLevel.NONE) {
			...
			if (getPunishLevel() == PunishLevel.JAIL)
				if (!isInsideZone(ZoneId.JAIL)) // If player escaped, put him back in jail
					teleToLocation(Location.JAIL);
		}
	}
	...
	public void calculateDeathPenaltyLevel(Creature killer) {
		...
		if (getCharmOfLuck() && (killer == null || killer.isRaidRelated()))
			return;
	...
	public void penaltyDeadPlayer(Player activeChar, String name, boolean ban) {
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
